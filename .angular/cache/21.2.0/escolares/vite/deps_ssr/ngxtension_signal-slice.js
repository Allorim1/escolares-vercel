...
import {
  ...
  signal,
  untracked
} from "./chunk-CLDOWMG2.js";
import {
  require_cjs
} from "./chunk-EZ2PHHIB.js";
import {
  __spreadValues,
  __toESM
    ...
    return observable.pipe(takeUntilDestroyed(destroyRef)).subscribe((x) => {
      const update = () => {
        signal2.update((prev) => {
          ...
          return __spreadValues(__spreadValues({}, prev), curr);
        });
      };
      if (useUntracked) {
        untracked(update);
...
function parseArgs(args) {
  ...
  if (args.length === 3) {
    if (typeof args[2] === "boolean") {
      if ((0, import_rxjs2.isObservable)(args[0])) {
        return [
          args[0],
          null,
          args[1],
...
// node_modules/ngxtension/fesm2022/ngxtension-create-notifier.mjs
...
function signalSlice(config) {
  ...
  for (const [key, actionSource] of Object.entries(actionSources)) {
    if ((0, import_rxjs3.isObservable)(actionSource)) {
      ...
      const sharedObservable = observable.pipe((0, import_rxjs3.share)());
...
function connectSources(state, sources, readonlyState, injector, useUntracked = false) {
  for (const source of sources) {
    if ((0, import_rxjs3.isObservable)(source)) {
...
function addReducerProperties(readonlyState, state$, key, destroyRef, subject, subs, observableFromActionSource) {
  ...
  Object.defineProperties(readonlyState, {
    [key]: {
      value: (nextValue) => {
        if ((0, import_rxjs3.isObservable)(nextValue)) {
        ...
        return new Promise((res) => {
          state$.pipe((0, import_rxjs3.take)(1)).subscribe((val) => {
            res(val);
          });
          version.notify();
